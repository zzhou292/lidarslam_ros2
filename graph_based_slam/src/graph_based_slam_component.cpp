...
GraphBasedSlamComponent::GraphBasedSlamComponent(
    ...
    : Node("graph_based_slam", options), clock_(RCL_ROS_TIME),
      tfbuffer_(std::make_shared<rclcpp::Clock>(clock_)), listener_(tfbuffer_),
      broadcaster_(this) {
  ...
  get_parameter("range_of_searching_loop_closure",
  ...
  std::cout << "loop_detection_period[Hz]:" << loop_detection_period_
            << std::endl;
  std::cout << "threshold_loop_closure_score:" << threshold_loop_closure_score_
            << std::endl;
  std::cout << "distance_loop_closure[m]:" << distance_loop_closure_
            << std::endl;
  std::cout << "range_of_searching_loop_closure[m]:"
            << range_of_searching_loop_closure_ << std::endl;
  std::cout << "search_submap_num:" << search_submap_num_ << std::endl;
  std::cout << "num_adjacent_pose_cnstraints:" << num_adjacent_pose_cnstraints_
            << std::endl;
  std::cout << "use_save_map_in_loop:" << std::boolalpha
            << use_save_map_in_loop_ << std::endl;
  ...
  if (registration_method == "NDT") {
    ...
    boost::shared_ptr<pclomp::GeneralizedIterativeClosestPoint<pcl::PointXYZI,
                                                               pcl::PointXYZI>>
        gicp(new pclomp::GeneralizedIterativeClosestPoint<pcl::PointXYZI,
                                                          pcl::PointXYZI>());
  ...
  map_save_srv_ =
      create_service<std_srvs::srv::Empty>("map_save", map_save_callback);
}
...
void GraphBasedSlamComponent::initializePubSub() {
  ...
  auto map_array_callback =
      [this](const typename lidarslam_msgs::msg::MapArray::SharedPtr msg_ptr)
      -> void {
...
void GraphBasedSlamComponent::searchLoop() {
  ...
  if (is_map_array_updated_ == false) {
    return;
  }
  if (map_array_msg_.cloud_coordinate != map_array_msg_.LOCAL) {
    RCLCPP_WARN(get_logger(),
                "cloud_coordinate should be local, but it's not local.");
  }
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr latest_submap_cloud_ptr(
  ...
  pcl::transformPointCloud(*latest_submap_cloud_ptr,
                           *transformed_latest_submap_cloud_ptr,
  ...
  Eigen::Vector3d latest_submap_pos{latest_submap.pose.position.x,
  ...
  if (is_candidate) {
    ...
    for (int j = 0; j <= 2 * search_submap_num_; ++j) {
      if (id_min + j - search_submap_num_ < 0) {
        continue;
      }
      ...
      pcl::transformPointCloud(*submap_cloud_ptr, *transformed_submap_cloud_ptr,
    ...
    if (fitness_score < threshold_loop_closure_score_) {
      ...
      std::cout << "distance:" << min_submap.distance
...
void GraphBasedSlamComponent::doPoseAdjustment(
    lidarslam_msgs::msg::MapArray map_array_msg, bool do_save_map) {
  ...
  for (int i = 0; i < submaps_size; i++) {
    ...
    if (i > num_adjacent_pose_cnstraints_) {
      for (int j = 0; j < num_adjacent_pose_cnstraints_; j++) {
        ...
        edge_se3->vertices()[0] =
            optimizer.vertex(i - num_adjacent_pose_cnstraints_ + j);
  ...
  for (int i = 0; i < submaps_size; i++) {
    ...
    pcl::transformPointCloud(*cloud_ptr, *transformed_cloud_ptr,
